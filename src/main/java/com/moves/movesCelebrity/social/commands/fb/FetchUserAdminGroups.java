...
public class FetchUserAdminGroups implements Command<ArrayList<Document>, String> {

    public FetchUserAdminGroups() {
    }
    @Override
    public CompletableFuture<ArrayList<Document>> execute(String arg) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            try {
                posts = fetchFbUserAdminGroups(arg);
            } catch (Exception e) {
                e.printStackTrace();
            }
            return posts;
    ...
    private ArrayList<Document> fetchFbUserAdminGroups(String accessToken) {

        ArrayList<Document> posts = null;
        String url = String.format(Constants.FACEBOOK_FETCH_ACCOUNTS_USER_GROUPS, accessToken);
