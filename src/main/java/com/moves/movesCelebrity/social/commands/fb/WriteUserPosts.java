...
public class WriteUserPosts implements Command<Void, ArrayList<Document>> {
    ...
    private void insert(ArrayList<Document> documents) {
        if (documents != null && documents.size() != 0) {
            SocialMediaPostDao.insertMany(documents, MovesConfiguration.COLLECTIONS_ACCOUNTS_USER_POSTS);
        }
    }
}
