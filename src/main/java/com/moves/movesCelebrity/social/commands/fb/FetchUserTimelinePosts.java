...
public class FetchUserTimelinePosts implements Command<ArrayList<Document>, String> {

    public FetchUserTimelinePosts() {
    }
    @Override
    public CompletableFuture<ArrayList<Document>> execute(String arg) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            try {
                posts = fetchFbUserTimelinePosts(arg);
            } catch (Exception e) {
                e.printStackTrace();
            }
            return posts;
    ...
    private ArrayList<Document> fetchFbUserTimelinePosts(String accessToken) {

        ArrayList<Document> posts = null;
        String url = String.format(Constants.FACEBOOK_FETCH_ACCOUNTS_USER_TIMELINE_POSTS, accessToken);
