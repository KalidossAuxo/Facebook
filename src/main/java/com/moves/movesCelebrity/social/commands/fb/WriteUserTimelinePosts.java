...
public class WriteUserTimelinePosts implements Command<Void, ArrayList<Document>> {
    ...
    private void insert(ArrayList<Document> documents) {
        if (documents != null && documents.size() != 0) {
            SocialMediaPostDao.insertMany(documents, MovesConfiguration.COLLECTIONS_ACCOUNTS_USER_FEED_POSTS);
        }
    }
}
